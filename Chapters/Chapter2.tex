...
Research on user interface evolution has long emphasized that adaptation mechanisms require dedicated conceptual tools to describe and reason about the malleability of interactive systems. Prior to the emergence of \textit{Extra-User Interfaces} (Extra-UIs) as a clearly identified class of interfaces supporting user control over adaptation, several efforts attempted to map the design space of UI plasticity.

A seminal precursor is the \textit{pStars} problem space proposed by Calvary et al.~\cite{Calvary:2011}, which characterizes the dimensions involved in user interface plasticity across interaction style, task structure, distribution, context of use, and adaptation behavior. Although this model was not intended as a design space for meta-UIs or Extra-UIs, it demonstrated that reasoning about UI evolution requires its own representational space. Figure~\ref{fig:pstars} reproduces this early attempt to structure the problem.
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\linewidth]{Images/pstars.png}
    \caption{The \textit{pStars} plasticity problem space~\cite{Calvary:2011}.}
    \label{fig:pstars}
\end{figure}
Building on this lineage, we define the notion of Extra-UI as follows:
\begin{quote}
\textbf{An Extra-UI is any user interface whose purpose is to support the software evolution of another user interface.}
\end{quote}

In the context of this thesis, we focus on a specific form of software evolution: \textbf{user interface adaptation occurring after the deployment} of an interactive system. This restriction imposes a practical constraint: \textbf{the Extra-UI must be shipped together with the interactive system it supervises}. It is therefore not an external developer tool, but a runtime component supporting end users.

To situate this notion within the broader family of “*-UIs,” we extend the classical list (meta-UI, mega-UI ) with the \textbf{supra-UI}. 

The supra-UI refers to a user interface that operates above multiple user interfaces, coordinating several target UIs or several adaptation mechanisms at a higher orchestration level \cite{Calvary:1997}. Unlike the meta-UI, which supervises a single UI, the supra-UI provides control across UI ensembles or multi-layer adaptation processes.
, as originally defined alongside the others in prior literature.

Extra-UIs provide a set of services whose nature depends on the type of UI they supervise. These include, among others, observation of adaptation decisions, configuration of rules, execution control, and evaluation of adaptation outcomes. Their scope is inherently dependent on the supervised UI. For instance, when the UI is \textit{distributed}, the Extra-UI must also expose services supporting distribution, which constitutes a particular form of adaptation.

Recent work further illustrates the diversity of Extra-UIs. For example,    \cite{dutronc:2025}, who designed a meta-user interface woven directly inside a web-based PLM system to allow workers to personalize their environment at runtime. Their approach, implemented through JavaScript injection, illustrates how an Extra-UI can be shipped with an interactive system and provide end users with direct adaptation control.

Building on these foundations, this chapter introduces a complete design space for Extra-User Interfaces, articulating the functions, structures, and interaction techniques required for users to observe, configure, and control UI adaptation in a transparent and intelligible manner.
...
\begin{itemize}[noitemsep]
    \item For the manipulated objects dial: nature of objects and type of object visualization,
    \item For the power quadrant: services offered and extensibility of the interaction language,
    \item For the qualities quadrant: level of categorization and level of user control.
\end{itemize}
...
The variables identify what has changed in the context of use. According to the ontology of Crowley \etal \cite{Crowley:2002}, four types of changes can occur depending on whether they affect \textsc{entities}, \textsc{roles}, \textsc{relations}, or \textsc{associations} between entities, roles, and relations:
\begin{figure}
        \centering
...
When a smartphone is held vertically in portrait mode, the calculator default configuration displays only simple arithmetic operations (\autoref{fig:calculatorModes}a). When the smartphone is turned horizontally in landscape mode, it provides more space to display sophisticated scientific functions, such as trigonometric functions (\autoref{fig:calculatorModes}b). The underlying components remain the same, but the orientation affects how they are displayed and arranged. This modification is a representative example of an adaptation operation that modifies a property. In contrast, \textsc{MiniAba}~\cite{Schlee:2004} enables the end users to (un)select any subset of arithmetic, trigonometric, or advanced functions they want and to recompile the project to get an adapted version.
...
\begin{figure}
    ...
    \caption{Evaluation of an adaptation operation.}
