...
\begin{itemize}
    \item To present a model-based method for engineering Extra-UIs, grounded in explicit models of users, adaptations, and adaptation operations.
...
The chapter is structured as follows. Section~\ref{sec:chap4-from-design-space} connects the design space to the requirements and constraints of the SYMBIOTIK project. Section~\ref{sec:chap4-model-driven} presents the underlying models that drive the Extra-UI: user models, adaptation models, and adaptation operations. Section~\ref{sec:chap4-arch} introduces the architecture of the SYMBIOTIK adaptation engine and its Extra-UI. Section~\ref{sec:chap4-usecase-panel} details the SYMBIOTIK adaptation panel as a concrete Extra-UI that guides users through a seven-stage adaptation cycle. Section~\ref{sec:chap4-evolution} describes two successive implementations of the adaptation engine, including the current version used for a user study and for integration with a reinforcement learning (RL) module. Section~\ref{sec:chap4-discussion} discusses the main design lessons with respect to user control, transparency, and the design space. Section~\ref{sec:chap4-conclusion} concludes and prepares the transition toward the evaluation of Extra-UIs in the next chapter.
...
These requirements articulate the expectations placed on Extra-UIs in a real-world, multi-partner European project. They guide the model-based approach presented in the next section.

\section{Model-Based Methods for Extra-UIs}
...
The first pillar of the model-based method is an explicit user model. In SYMBIOTIK, the user model is captured by a \texttt{User} class and related classes, represented in a UML diagram (Figure~\ref{fig:symbiotik-user-class}). This class encodes both static and dynamic user attributes relevant to UI adaptation:
\begin{itemize}[noitemsep]
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-user-class}
    \caption{UML diagram of the SYMBIOTIK \texttt{User} class and related classes.}
    \label{fig:symbiotik-user-class}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-adaptation-class}
    \caption{UML diagram of the \texttt{Adaptation} class and its core subcomponents.}
    \label{fig:symbiotik-adaptation-class}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-architecture}
    \caption{Architecture of the SYMBIOTIK adaptation engine, showing the intelligent driver, adaptation engine, AVT, and external services.}
    \label{fig:symbiotik-architecture}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=0.7\linewidth]{figures/symbiotik-signin}
    \caption{Sign-in page of the SYMBIOTIK adaptation panel.}
    \label{fig:symbiotik-signin}
\end{figure}

\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=0.7\linewidth]{figures/symbiotik-signup}
    \caption{Sign-up page of the SYMBIOTIK adaptation panel.}
    \label{fig:symbiotik-signup}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-profile-general}
    \caption{General section of the user profile in the SYMBIOTIK adaptation panel.}
    \label{fig:symbiotik-profile-general}
\end{figure}

\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-profile-preferences}
    \caption{Preferences section of the user profile, where users configure their adaptation preferences.}
    \label{fig:symbiotik-profile-preferences}
\end{figure}

\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-profile-devices}
    \caption{Sensing devices section of the user profile, where available devices are selected.}
    \label{fig:symbiotik-profile-devices}
\end{figure}

\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-profile-measures}
    \caption{Measures section of the user profile, where monitored measures are configured.}
    \label{fig:symbiotik-profile-measures}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-goal}
    \caption{Goal configuration section in the SYMBIOTIK adaptation panel, where users define the objective of the adaptation.}
    \label{fig:symbiotik-goal}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-initiative}
    \caption{Initiative configuration section, where users select who triggers adaptations and under which conditions.}
    \label{fig:symbiotik-initiative}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-specification}
    \caption{Specification section of the adaptation panel, where users select and configure adaptation operations.}
    \label{fig:symbiotik-specification}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-application}
    \caption{Application section, where users control how and when the selected operations are applied.}
    \label{fig:symbiotik-application}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-transition}
    \caption{Transition configuration section, where users select how adaptations are visually introduced.}
    \label{fig:symbiotik-transition}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-interpretation}
    \caption{Interpretation section, showing feedback on how measures evolved after adaptation.}
    \label{fig:symbiotik-interpretation}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    % TODO: \includegraphics[width=\linewidth]{figures/symbiotik-evaluation}
    \caption{Evaluation section, where users assess the adaptation and decide on acceptance or further changes.}
    \label{fig:symbiotik-evaluation}
\end{figure}
...
\section{Evolution of the Adaptation Engine Through Two Implementations}
...
This implementation validated the feasibility of using a model-based approach to connect user profiles, adaptation goals, and concrete operations on dashboards. It also provided early feedback on the usability of the Extra-UI for editing complex adaptation models.

\subsection{Second implementation: user study engine and RL-integrated engine}
\label{subsec:chap4-second-impl}

The second implementation, which is the current version in SYMBIOTIK, consists of two closely related variants of the adaptation engine. Both use the same core architecture (React front-end, Node.js/Express or NestJS back-end, MongoDB, Kafka, Socket.IO, and AVT), but differ in how adaptation operations are triggered and selected.
...
This chapter has operationalized the concept of Extra-User Interfaces introduced in earlier chapters by presenting a model-based method, architecture, and concrete use cases in the context of the SYMBIOTIK project. The SYMBIOTIK adaptation panel exemplifies how an Extra-UI can be shipped with an interactive system to govern a full adaptation life cycle, while preserving user control, transparency, and traceability.
